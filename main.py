...
os.system(f"xterm -fa 'Monospace' -fs 17 -rightbar -into {wid} -geometry {var['win_h']}x{var['win_w']-50} -bg {var['color_bg']} -fg {var['color_fg']} -sb -e 'clear && /usr/bin/python -q && echo no > {dir}/pypad/exists' &")

if(sys("cat {}/pypad/exists".format(dir)) == 'no'):
    sys(f"echo yes > {dir}/pypad/exists")
    quit()
